...
//  ==========================================================================
// 
//  $LastChangedRevision: 12797 $
//  $LastChangedDate: 2011-11-19 21:13:58 +0600 (Сб, 19 ноя 2011) $
...
{
    //  The library configuration parameters.
    public static class LibVlcPresentationConfig
    {
        //  The motion detector sensitivity.
    ...
    public class MediaElement
        : FrameworkElement
    {
        private static readonly ConcurrentDictionary<MediaPlayer, WeakReference<MediaElement>> m_MediaElements =
            new ConcurrentDictionary<MediaPlayer, WeakReference<MediaElement>>();

        private static readonly LibVLCLibrary m_Library;

        private VideoFragmentManifest _manifest;
        private MotionEventDetector _motionEvent;
        ...
        public VideoFragmentManifest Manifest
        {
        ...
        {
            var media_player = sender as MediaPlayer;
        ...
        private void MediaPlayer_TimeChanged()
        {
            if (!IsOpen)
            {
                ...
                if (VideoStreams == null)
                {
                    ...
                    CurrentVideoStream = ((track_index < 0) || (track_index >= VideoStreams.Length))
                                             ? null
                                             : VideoStreams[track_index];
                }
                if (AudioStreams == null)
                {
                    AudioStreams = MediaPlayer.AudioTracks.Select((t) => new AudioStream(t)).ToArray();
                    int track_index = MediaPlayer.AudioTrackIndex;
                    CurrentAudioStream = ((track_index < 0) || (track_index >= AudioStreams.Length))
                                             ? null
                                             : AudioStreams[track_index];
                }
                if (SubtitleStreams == null)
                {
                    ...
                    CurrentSubtitleStream = ((track_index < 0) || (track_index >= SubtitleStreams.Length))
                                                ? null
                                                : SubtitleStreams[track_index];
                }
        ...
        {
            var videoBuffer = MediaPlayer.VideoBuffer;
            if (videoBuffer != null)
            {
                //  The intermediate bitmap for video rendering.
                VideoBufferBitmap = new WriteableBitmap((int) videoBuffer.Width, (int) videoBuffer.Height, 96, 96, PixelFormats.Bgra32, null);
                
                _motionEvent = new MotionEventDetector(_manifest,
#if _USE_BIT_ARRAY
                                                      new BitArrayContainer(videoBuffer.FrameBuffer, (int)videoBuffer.Width, (int)videoBuffer.Height, (int)videoBuffer.Stride),
#else
                                                      new WriteableBitmapContainer(VideoBufferBitmap),
#endif
                                                      LibVlcPresentationConfig.Sensitivity
                                                      );
                FPS = MediaPlayer.FPS;
            }
            else
        ...
        private void MediaPlayer_VideoDisplay()
        {
            ...
            if ((VideoBufferBitmap.PixelWidth != videoBuffer.Width) ||
                (VideoBufferBitmap.PixelHeight != videoBuffer.Height))
                return;

            if (RenderingEnabled)
            {
                VideoBufferBitmap.WritePixels(new Int32Rect(0, 0, (int) videoBuffer.Width, (int) videoBuffer.Height), videoBuffer.FrameBuffer, (int) videoBuffer.Stride, 0);
            }
            ...
            //  Fills the start time as the real time of rendering of the first frame!
            if (m_FrameCounter == 1) m_SecondStart = now;

            var seconds = (now - m_SecondStart).TotalSeconds;
            if (seconds >= 2.0)
            {
                //  Skips first 3 seconds of irrelevant video data.
                if (_motionEvent.EventOccured(
#if _USE_BIT_ARRAY
                    new BitArrayContainer(video_buffer.FrameBuffer, (int)video_buffer.Width, (int)video_buffer.Height, (int)video_buffer.Stride)
#else
                    new WriteableBitmapContainer(VideoBufferBitmap)
#endif
                    ))
                {
                    Debug.WriteLine("MediaPlayer_VideoDisplay - motion detected!", "MediaElement");
                }
            ...
            {
                Debug.WriteLine("Total seconds: " + seconds);
            }
        }
        ...
        {
            if (VideoBufferBitmap != null && RenderingEnabled)
            {
                ...
                drawingContext.DrawImage(VideoBufferBitmap, new Rect(
                                                                ...
                                                                (RenderSize.Height - size.Height)/2, size.Width,
                                                                size.Height));
                drawingContext.Pop();
            }
        ...
        //==========================================================================
        /// <summary>
        ///     Starts immediately playback of given source.
        /// </summary>
        /// <exception cref="InvalidOperationException">
        ///     Will be thrown if no media has been loaded.
        /// </exception>
        ...
        {
            // At this point MediaPlayer can be null.
            // OnSourceChange sets up the MediaPlayer.
        ...
        private static readonly DependencyPropertyKey VideoBufferBitmapPropertyKey =
            DependencyProperty.RegisterReadOnly("VideoBufferBitmap", typeof (WriteableBitmap), typeof (MediaElement),
        ...
        //==========================================================================
        private static readonly DependencyProperty VideoBufferBitmapProperty =
            VideoBufferBitmapPropertyKey.DependencyProperty;

        private WriteableBitmap VideoBufferBitmap
        {
            get { return (WriteableBitmap) GetValue(VideoBufferBitmapProperty); }

            set { SetValue(VideoBufferBitmapPropertyKey, value); }
        }
        ...
        private static readonly DependencyPropertyKey IsOpenPropertyKey =
            DependencyProperty.RegisterReadOnly("IsOpen", typeof (bool), typeof (MediaElement),
        ...
        private static readonly DependencyPropertyKey MediaPlayerPropertyKey =
            DependencyProperty.RegisterReadOnly("MediaPlayer", typeof (MediaPlayer), typeof (MediaElement),
        ...
        private MediaPlayer MediaPlayer
        {
            ...
            set { SetValue(MediaPlayerPropertyKey, value); }
        }
        ...
        private static readonly DependencyPropertyKey ActualFPSPropertyKey =
            DependencyProperty.RegisterReadOnly("ActualFPS", typeof (double?), typeof (MediaElement),
                                                new FrameworkPropertyMetadata(null));
        ...
        private DateTime m_SecondStart;
        ...
        {
            get { return (double?) GetValue(ActualFPSProperty); }

            private set { SetValue(ActualFPSPropertyKey, value); }
        }
        ...
        private static readonly DependencyPropertyKey FPSPropertyKey =
            DependencyProperty.RegisterReadOnly("FPS", typeof (double?), typeof (MediaElement),
                                                new FrameworkPropertyMetadata(null));
        ...
        private static readonly DependencyPropertyKey ActualSourcePropertyKey =
            DependencyProperty.RegisterReadOnly("ActualSource", typeof (Uri), typeof (MediaElement),
                                                new FrameworkPropertyMetadata(null));
        ...
        private static readonly DependencyPropertyKey LengthPropertyKey =
            DependencyProperty.RegisterReadOnly("Length", typeof (TimeSpan?), typeof (MediaElement),
                                                new FrameworkPropertyMetadata(null, OnLengthChanged));
        ...
        public static readonly DependencyProperty PositionProperty =
            DependencyProperty.Register("Position", typeof (TimeSpan?), typeof (MediaElement),
                                        new FrameworkPropertyMetadata(null, OnPositionChanged, CoercePosition));
        ...
        public static readonly DependencyProperty VolumeProperty =
            DependencyProperty.Register("Volume", typeof (double), typeof (MediaElement),
                                        new FrameworkPropertyMetadata(1.0, OnVolumeChanged, CoerceVolume));
        ...
        private static readonly DependencyPropertyKey StatePropertyKey =
            DependencyProperty.RegisterReadOnly("State", typeof (MediaElementState), typeof (MediaElement),
                                                new FrameworkPropertyMetadata(MediaElementState.Empty, OnStateChanged));
        ...
        private static readonly DependencyPropertyKey VideoStreamsPropertyKey =
            DependencyProperty.RegisterReadOnly("VideoStreams", typeof (VideoStream[]), typeof (MediaElement),
                                                new FrameworkPropertyMetadata(null, OnVideoStreamsChanged));
        ...
        {
            get { return (VideoStream[]) GetValue(VideoStreamsProperty); }

            private set { SetValue(VideoStreamsPropertyKey, value); }
        }
        ...
        public static readonly DependencyProperty CurrentVideoStreamProperty =
            DependencyProperty.Register("CurrentVideoStream", typeof (VideoStream), typeof (MediaElement),
                                        new FrameworkPropertyMetadata(null, OnCurrentVideoStreamChanged,
                                                                      CoerceCurrentVideoStream));
        ...
        {
            get { return (VideoStream) GetValue(CurrentVideoStreamProperty); }

            set { SetValue(CurrentVideoStreamProperty, value); }
        }
        ...
        private static readonly DependencyPropertyKey AudioStreamsPropertyKey =
            DependencyProperty.RegisterReadOnly("AudioStreams", typeof (AudioStream[]), typeof (MediaElement),
                                                new FrameworkPropertyMetadata(null, OnAudioStreamsChanged));
        ...
        {
            get { return (AudioStream[]) GetValue(AudioStreamsProperty); }

            private set { SetValue(AudioStreamsPropertyKey, value); }
        }
        ...
        public static readonly DependencyProperty CurrentAudioStreamProperty =
            DependencyProperty.Register("CurrentAudioStream", typeof (AudioStream), typeof (MediaElement),
                                        new FrameworkPropertyMetadata(null, OnCurrentAudioStreamChanged,
                                                                      CoerceCurrentAudioStream));
        ...
        {
            get { return (AudioStream) GetValue(CurrentAudioStreamProperty); }

            set { SetValue(CurrentAudioStreamProperty, value); }
        }
        ...
        private static readonly DependencyPropertyKey SubtitleStreamsPropertyKey =
            DependencyProperty.RegisterReadOnly("SubtitleStreams", typeof (SubtitleStream[]), typeof (MediaElement),
        ...
        /// </summary>
        public static readonly DependencyProperty SubtitleStreamsProperty =
            SubtitleStreamsPropertyKey.DependencyProperty;

        public SubtitleStream[] SubtitleStreams
        {
            get { return (SubtitleStream[]) GetValue(SubtitleStreamsProperty); }

            private set { SetValue(SubtitleStreamsPropertyKey, value); }
        }
        ...
        {
            (sender as MediaElement).OnSubtitleStreamsChanged((SubtitleStream[]) e.OldValue,
                                                              (SubtitleStream[]) e.NewValue);
        }
        ...
        public static readonly DependencyProperty CurrentSubtitleStreamProperty =
            DependencyProperty.Register("CurrentSubtitleStream", typeof (SubtitleStream), typeof (MediaElement),
                                        new FrameworkPropertyMetadata(null, OnCurrentSubtitleStreamChanged,
                                                                      CoerceCurrentSubtitleStream));

        public SubtitleStream CurrentSubtitleStream
        {
            get { return (SubtitleStream) GetValue(CurrentSubtitleStreamProperty); }

            set { SetValue(CurrentSubtitleStreamProperty, value); }
        }
        ...
        {
            (sender as MediaElement).OnCurrentSubtitleStreamChanged((SubtitleStream) e.OldValue,
                                                                    (SubtitleStream) e.NewValue);
        }
        ...
        private static readonly DependencyPropertyKey ChapterCountPropertyKey =
            DependencyProperty.RegisterReadOnly("ChapterCount", typeof (int?), typeof (MediaElement),
                                                new PropertyMetadata(default(int?), OnChapterCountChanged,
                                                                     CoerceChapterCount));
        ...
        {
            get { return (int?) GetValue(ChapterCountProperty); }

            private set { SetValue(ChapterCountPropertyKey, value); }
        }
        ...
        public static readonly DependencyProperty CurrentChapterProperty =
            DependencyProperty.Register("CurrentChapter", typeof (int?), typeof (MediaElement),
                                        new FrameworkPropertyMetadata(null, OnCurrentChapterChanged,
                                                                      CoerceCurrentChapter));
        ...
        {
            get { return (int?) GetValue(CurrentChapterProperty); }
        ...
        /// </summary>
        public static readonly RoutedEvent PlayingEvent = EventManager.RegisterRoutedEvent("Playing",
                                                                                           RoutingStrategy.Direct,
                                                                                           typeof (RoutedEventHandler),
                                                                                           typeof (MediaElement));
        ...
        /// </summary>
        public static readonly RoutedEvent PausedEvent = EventManager.RegisterRoutedEvent("Paused",
                                                                                          RoutingStrategy.Direct,
                                                                                          typeof (RoutedEventHandler),
                                                                                          typeof (MediaElement));
        ...
        /// </summary>
        public static readonly RoutedEvent StoppedEvent = EventManager.RegisterRoutedEvent("Stopped",
                                                                                           RoutingStrategy.Direct,
                                                                                           typeof (RoutedEventHandler),
                                                                                           typeof (MediaElement));
        ...
        /// </summary>
        public static readonly RoutedEvent EndReachedEvent = EventManager.RegisterRoutedEvent("EndReached",
                                                                                              RoutingStrategy.Direct,
                                                                                              typeof (RoutedEventHandler
                                                                                                  ),
                                                                                              typeof (MediaElement));
        ...
        /// </summary>
        public static readonly RoutedEvent EncounteredErrorEvent = EventManager.RegisterRoutedEvent("EncounteredError",
                                                                                                    RoutingStrategy
                                                                                                        .Direct,
                                                                                                    typeof (
                                                                                                        RoutedEventHandler
        ...
        /// </summary>
        public static readonly RoutedEvent PositionChangedEvent = EventManager.RegisterRoutedEvent("PositionChanged",
                                                                                                   RoutingStrategy
                                                                                                       .Direct,
                                                                                                   typeof (
                                                                                                       RoutedEventHandler
